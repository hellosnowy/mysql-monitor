...
export async function captureSnapshot(connID, forceBaseline, description) {
  if (hasWails()) {
    return await window.go.main.App.CaptureSnapshot(
      connID,
      forceBaseline,
      description,
    );
  ...
  const ver = {
    ...
    description:
      description || (isBaseline ? "初始基准版本 (Baseline)" : "快照"),
    table_count: 5,
    row_count: 120,
...
export async function compareVersions(connID, fromVersionID, toVersionID) {
  if (hasWails()) {
    return await window.go.main.App.CompareVersions(
      connID,
      fromVersionID,
      toVersionID,
    );
